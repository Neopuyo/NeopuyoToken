...
// Start a test series named FiveToken, it will use 10 test accounts 
contract("FiveToken", async accounts => {
    address0 = '0x0000000000000000000000000000000000000000'

    // each it is a new test, and we name our first test initial supply
    it("initial supply", async () => {
        // wait until fiveToken is deployed, store the results inside fiveToken
        // the result is a client to the Smart contract api
        let fiveToken = await FiveToken.deployed();
        // call our totalSupply function
        let supply = await fiveToken.totalSupply()

        // Assert that the supply matches what we set in migration
        assert.equal(supply.toNumber(), 5000000000, "Initial supply was not the same as in migration")
        console.log("initial supply 5M : OK !")
        // 5 000 000 000
    });
    ...
    it("minting", async() => {
        ...
        let intial_balance = await fiveToken.balanceOf(accounts[1]);
        ...
        let totalSupply = await fiveToken.totalSupply();
        await fiveToken.mint(accounts[1], 100);
        // Grab the balance again to see what it is after calling mint
        let after_balance = await fiveToken.balanceOf(accounts[1]);
        let after_supply = await fiveToken.totalSupply();
        ...
        try {
            // Mint with address 0
            await fiveToken.mint(address0, 100);
        } catch(error) {
            assert.equal(error.reason, "FiveToken: cannot mint to zero address", "Failed to stop minting on zero address")
        }
    })
    ...
    it("burning", async() => {
        ...
        let initial_balance = await fiveToken.balanceOf(accounts[1]);

        // Burn to address 0 
        try {
            await fiveToken.burn(address0, 100);
        } catch(error) {
            assert.equal(error.reason, "FiveToken: cannot burn from zero address", "Failed to notice burning on 0 address")
        }

        // Burn more than balance
        try {
            await fiveToken.burn(accounts[1], initial_balance+initial_balance);
        } catch(error) {
            assert.equal(error.reason, "FiveToken: Cannot burn more than the account owns", "Failed to capture too big burns on an account")
        }

        let totalSupply = await fiveToken.totalSupply();
        try {
            await fiveToken.burn(accounts[1], initial_balance - 50);
        } catch(error) {
            assert.fail(error);
        }

        let balance = await fiveToken.balanceOf(accounts[1]);
        ...
        let newSupply = await fiveToken.totalSupply();
    ...
    it("transfering tokens", async() => {
        ...
        await fiveToken.transfer(accounts[1], 100);
        ...
        let account2_initial_balance = await fiveToken.balanceOf(accounts[2]);

        await fiveToken.transfer(accounts[2], 20, { from: accounts[1]});
        // Make sure balances are switched on both accounts
        let account2_after_balance = await fiveToken.balanceOf(accounts[2]);
        let account1_after_balance = await fiveToken.balanceOf(accounts[1]);
        ...
        try {
            await fiveToken.transfer(accounts[2], 2000000000000, { from:accounts[1]});
        } catch(error) {
            assert.equal(error.reason, "FiveToken: cant transfer more than your account holds");
        }
    ...
    it ("allow account some allowance", async() => {
        ...
        try{
            ...
            await fiveToken.approve(address0, 100);    
        }catch(error){
            assert.equal(error.reason, 'FiveToken: approve cannot be to zero address', "Should be able to approve zero address");
        }

        try{
            // Give account 1 access too 100 tokens on zero account
            await fiveToken.approve(accounts[1], 100);    
        }catch(error){
            assert.fail(error); // shold not fail
        }

        // Verify by checking allowance
        let allowance = await fiveToken.allowance(accounts[0], accounts[1]);
    ...
    it("transfering with allowance", async() => {
        ...
        try{
            ...
            // lets try using more 
            await fiveToken.transferFrom(accounts[0], accounts[2], 200, { from: accounts[1] } );
        } catch(error) {
            assert.equal(error.reason, "FiveToken: You cannot spend that much on this account", "Failed to detect overspending")
        }
        let init_allowance = await fiveToken.allowance(accounts[0], accounts[1]);
        console.log("init balance: ", init_allowance.toNumber())
        try {
            // Account 1 should have 100 tokens by now to use on account 0 
            // lets try using more 
            let worked = await fiveToken.transferFrom(accounts[0], accounts[2], 50, {from:accounts[1]});
        } catch(error) {
            assert.fail(error);
        }
