...
contract("Stakeable_2", async accounts => {
    it("Staking 100x2", async () => {
        fiveToken = await FiveToken.deployed();
        ...
        await fiveToken.mint(accounts[1], 1000);
        // Get init balance of user
        balance = await fiveToken.balanceOf(owner)

        // Stake the amount, notice the FROM parameter which specifes what the msg.sender address will be

        stakeID = await fiveToken.stake(stake_amount, { from: owner });
        // Assert on the emittefiveent using truffleassert
    ...
    it("new stakeholder should have increased index", async () => {
        let stake_amount = 100;
        stakeID = await fiveToken.stake(stake_amount, { from: accounts[1] });
        // Assert on the emittefiveent using truffleassert
        // This will capture the event and inside the event callback we can use assert on the values returned
        truffleAssert.eventEmitted(
            stakeID,
    ...
    it("cannot stake more than owning", async () => {
        ...
        try {
            await fiveToken.stake(1000000000, { from: accounts[2] });
        } catch (error) {
    ...
    it("cant withdraw bigger amount than current stake", async() => {
        ...
        try {
            await fiveToken.withdrawStake(200, 0, {from:owner});
        } catch(error) {
    ...
    it("withdraw 50 from a stake", async() => {
        ...
        await fiveToken.withdrawStake(withdraw_amount, 0, {from:owner});
        
        let summary = await fiveToken.hasStake(owner); // Grab a new summary to see if the total amount has changed
    ...
    it("remove stake if empty", async() => {
        ...
        let summary = await fiveToken.hasStake(owner);
        console.log(summary);
    ...
    it("calculate rewards", async() => {
        ...
        await fiveToken.stake(1000, {from: owner});
        await helper.advanceTimeAndBlock(3600*20);

        summary = await fiveToken.hasStake(owner);
    ...
    it("reward stakes", async() => {
        ...
        await fiveToken.mint(accounts[3],1000);
        let initial_balance = await fiveToken.balanceOf(staker);
        // Make a stake on 200, fast forward 20 hours, claim reward, amount should be Initial balanace +4
        await fiveToken.stake(200, {from: staker});
        await helper.advanceTimeAndBlock(3600*20);

        let stakeSummary = await fiveToken.hasStake(staker);
        let stake = stakeSummary.stakes[0];
        // Withdraw 100 from stake at index 0 
        await fiveToken.withdrawStake(100, 0, { from: staker});

        // Balance of account holder should be updated by 104 tokens
        let after_balance = await fiveToken.balanceOf(staker);
        ...
        let second_balance = await fiveToken.balanceOf(staker);
